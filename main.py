...
class CommandBot(sleekxmpp.ClientXMPP):
    def __init__(
        ...
        self.stemmer = SnowballStemmer("english") if SnowballStemmer else None
    ...
    def start(self, event):
        ...
        # Join the rooms (skip any that are not configured)
        ...
        if self.spam_reports:
            self.send_message(
                mto=self.spam_reports,
                mbody="GinieBot is back online.",
                mtype="groupchat",
            )
    ...
    def search_wikipedia(self, query):
        if wikipedia is None or not _ensure_nltk_punkt():
        ...
        try:
            ...
            if self.nick not in names_list:
                if OpenAI is None or not os.getenv("OPENAI_API_KEY"):
                    return "AI replies are currently unavailable."
                client = OpenAI()
                completion = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "user",
                            "content": f"{greeting}, {sender_nick.replace(self.nick, 'Geniebot')}. {prompt}",
                        }
                    ],
                    max_tokens=120,
                    temperature=0.6,
    ...
    def post_to_forum(self, msg_text):
        if webdriver is None:
            self.send_message(
                mto=self.spam_reports,
                mbody="Forum posting unavailable: selenium is not installed.",
                mtype="groupchat",
            )
            return
        options = Options()
        ...
        username.send_keys(os.getenv("FORUM_USER", ""))
        password.send_keys(os.getenv("FORUM_PASS", ""))
        submit_button.click()
    ...
    def update_chart(self):
        if go is None or make_subplots is None:
            logging.warning("Skipping chart update: plotly is not installed")
            return
    ...
    def message(self, msg):
        ...
        if msg == "!users":
            ...
            stem_fn = (lambda w: self.stemmer.stem(w)) if self.stemmer else (lambda w: w)
            for lang_code, words in filter_words.items():
                for word in words:
                    stem = stem_fn(word)
                    regex = re.compile(f"\\b{stem}\\b", re.IGNORECASE)
                    if any(regex.search(stem_fn(w)) for w in message_words):
...
if __name__ == "__main__":
    ...
    nick = os.getenv("XMPP_NICKNAME", "GinieBot")
    room = os.getenv("XMPP_ROOM", "arena26@conference.lobby.wildfiregames.com")

    missing = [name for name, val in (("XMPP_JID", jid), ("XMPP_PASSWORD", password)) if not val]
    if missing:
    ...
    xmpp = CommandBot(
        ...
        room,
        nick,
        default_target_room=os.getenv("XMPP_DEFAULT_TARGET_ROOM", room),
        spam_reports=os.getenv("XMPP_SPAM_REPORTS", ""),
        arena25=os.getenv("XMPP_ARENA25", "arena25@conference.lobby.wildfiregames.com"),
        arena27=os.getenv("XMPP_ARENA27", "arena27@conference.lobby.wildfiregames.com"),
    )
